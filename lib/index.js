...
class MongoDBHintProvider {
  ...
  execute(cm) {
    ...
    const isTokenMatching = (matches) => {
      return matches.includes(token.string);
    };
    ...
    const isValueSuggestable = () => {
      const matches = [ COLON ];
      return (isPrevTokenMatching(matches) || isSkipPrevTokenMatching(matches));
    };
    ...
    const enhanceResult = (result) => {
      if (result.list.length === 1 && result.list[0].text === token.string) {
        result.list = [ this.normalizeSuggestion(token.string + ': ', token) ];
        result._case = 'start the next clause';
      }
      return result;
    };
    ...
    if (isValueSuggestable()) {
      // if (token.type === 'string' &&
          // _.startsWith(token.string, "'") && !_.endsWith(token.string, "'")) {
